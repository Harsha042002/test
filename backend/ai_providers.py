...
class AIProvider(ABC):
    ...
    async def count_tokens(self, text: str) -> int:
        ...
        try:
            # First try to use Gemini's token counter if available
            if hasattr(genai, 'count_tokens'):
                result = genai.count_tokens(model=self.model, prompt=text)
                return result.total_tokens
        except Exception as e:
            print(f"Error counting tokens with Gemini using primary method: {e}")
            
        # Fallback to approximation method
        return int(len(text) * 0.25)  # Rough estimate: 4 chars ≈ 1 token
...
def count_tokens(text):
    tokenizer = get_encoding("cl100k_base")  # Replace with your tokenizer
